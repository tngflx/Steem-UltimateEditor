;(function ($, window, document, undefined) {
    ...
    Core.prototype.init = function () {
        this.$el.addClass('ultimate-editor-insert-plugin');
    ...
    Core.prototype.events = function () {
        ...
        this.$el
            ...
            .on('selectstart mousedown', '.ultimate-insert, .ultimate-insert-buttons', $.proxy(this, 'disableSelection'))
            .on('click', '.ultimate-insert-buttons-show', $.proxy(this, 'toggleAddons'))
            .on('click', '.ultimate-insert-action', $.proxy(this, 'addonAction'))
            .on('paste', '.ultimate-insert-caption-placeholder', function (e) {
    ...
    Core.prototype.editorSerialize = function () {
        ...
        $.each(data, function (key) {
            ...
            $data.find('.ultimate-insert-buttons').remove();
            $data.find('.ultimate-insert-active').removeClass('ultimate-insert-active');
    ...
    Core.prototype.editorUpdatePlaceholder = function (el, dontShow) {
        var contents = $(el).children()
            .not('.ultimate-insert-buttons').contents();
    ...
    Core.prototype.disable = function () {
        ...
        this.$el.find('.ultimate-insert-buttons').addClass('hide');
    };
    ...
    Core.prototype.enable = function () {
        ...
        this.$el.find('.ultimate-insert-buttons').removeClass('hide');
    };
    ...
    Core.prototype.disableSelection = function (e) {
        ...
        if ($el.is('img') === false || $el.hasClass('ultimate-insert-buttons-show')) {
            e.preventDefault();
        }
    };
    ...
    Core.prototype.clean = function () {
        ...
        $buttons = this.$el.find('.ultimate-insert-buttons');
        $lastEl = $buttons.prev();
        if ($lastEl.attr('class') && $lastEl.attr('class').match(/ultimate\-insert(?!\-active)/)) {
    ...
    Core.prototype.addButtons = function () {
        if (this.$el.find('.ultimate-insert-buttons').length === 0) {
    ...
    Core.prototype.toggleButtons = function (e) {
        ...
        if ($current.hasClass('ultimate-editor-insert-plugin')) {
        ...
        if ($el.hasClass('ultimate-editor-placeholder') === false && $el.closest('.ultimate-insert-buttons').length === 0 && $current.closest('.ultimate-insert-buttons').length === 0) {

            this.$el.find('.ultimate-insert-active').removeClass('ultimate-insert-active');

            $.each(this.options.addons, function (addon) {
                if ($el.closest('.ultimate-insert-' + addon).length) {
                    $current = $el;
                }

                if ($current.closest('.ultimate-insert-' + addon).length) {
                    $p = $current.closest('.ultimate-insert-' + addon);
                    activeAddon = addon;
                    return;
                }
            });
            ...
            if ($p.length && (($p.text().trim() === '' && !activeAddon) || activeAddon === 'images')) {
                $p.addClass('ultimate-insert-active');

                if (activeAddon === 'images') {
                    this.$el.find('.ultimate-insert-buttons').attr('data-active-addon', activeAddon);
                } else {
                    this.$el.find('.ultimate-insert-buttons').removeAttr('data-active-addon');
                }
    ...
    Core.prototype.showButtons = function (activeAddon) {
        var $buttons = this.$el.find('.ultimate-insert-buttons');
    ...
    Core.prototype.hideButtons = function ($el) {
        ...
        $el.find('.ultimate-insert-buttons').hide();
        $el.find('.ultimate-insert-buttons-addons').hide();
        $el.find('.ultimate-insert-buttons-show').removeClass('ultimate-insert-buttons-rotate');
    };
    ...
    Core.prototype.positionButtons = function (activeAddon) {
        var $buttons = this.$el.find('.ultimate-insert-buttons'),
            ...
            $lastCaption = $p.hasClass('ultimate-insert-images-grid') ? [] : $p.find('figure:last figcaption'),
        ...
        if ($p.length) {
            ...
            if (activeAddon) {
                position.left += $p.width() - $buttons.find('.ultimate-insert-buttons-show').width() - 10;
                position.top += $p.height() - 20 + ($lastCaption.length ? -$lastCaption.height() - parseInt($lastCaption.css('margin-top'), 10) : 10);
            } else {
                position.left += -parseInt($buttons.find('.ultimate-insert-buttons-addons').css('left'), 10) - parseInt($buttons.find('.ultimate-insert-buttons-addons button:first').css('margin-left'), 10);
            ...
            if (this.$el.hasClass('ultimate-editor-placeholder') === false && position.left < 0) {
                position.left = $p.position().left;
            }
    ...
    Core.prototype.toggleAddons = function () {
        if (this.$el.find('.ultimate-insert-buttons').attr('data-active-addon') === 'images') {
            this.$el.find('.ultimate-insert-buttons').find('button[data-addon="images"]').click();
            return;
        }

        this.$el.find('.ultimate-insert-buttons-addons').fadeToggle();
        this.$el.find('.ultimate-insert-buttons-show').toggleClass('ultimate-insert-buttons-rotate');
    };
    ...
    Core.prototype.hideAddons = function () {
        this.$el.find('.ultimate-insert-buttons-addons').hide();
        this.$el.find('.ultimate-insert-buttons-show').removeClass('ultimate-insert-buttons-rotate');
    };
    ...
    Core.prototype.removeCaptions = function ($ignore) {
        ...
        $captions.each(function () {
            if ($(this).hasClass('ultimate-insert-caption-placeholder') || $(this).text().trim() === '') {
                $(this).remove();
            }
        });
    };
    ...
    Core.prototype.removeCaptionPlaceholder = function ($el) {
        ...
        if ($caption.length) {
            $caption
                .removeClass('ultimate-insert-caption-placeholder')
    ...
    $.fn[pluginName] = function (options) {
        return this.each(function () {
            ...
            if ($(that).is('textarea')) {
                textareaId = $(that).attr('ultimate-editor-textarea-id');
                that = $(that).siblings('[ultimate-editor-textarea-id="' + textareaId + '"]').get(0);
            }
