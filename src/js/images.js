...
; (function ($, window, document, Util, undefined) {
    ...
    var pluginName = 'ultimateInsert',
        ...
        defaults = {
            ...
            sorting: function () {
                ...
                $('.ultimate-insert-images').sortable({
                    group: 'ultimate-insert-images',
                    containerSelector: '.ultimate-insert-images',
                    itemSelector: 'figure',
    ...
    Images.prototype.init = function () {
        var $images = this.$el.find('.ultimate-insert-images');
    ...
    Images.prototype.events = function () {
        $(document)
            ...
            .on('click', '.ultimate-insert-images-toolbar .ultimate-editor-action', $.proxy(this, 'toolbarAction'))
            .on('click', '.ultimate-insert-images-toolbar2 .ultimate-editor-action', $.proxy(this, 'toolbar2Action'));

        this.$el
            .on('click', '.ultimate-insert-images img', $.proxy(this, 'selectImage'));
    ...
    Images.prototype.backwardsCompatibility = function () {
        this.$el.find('.ultimateInsert')
            .removeClass('ultimateInsert')
            .addClass('ultimate-insert-images');

        this.$el.find('.ultimate-insert-images.small')
            .removeClass('small')
            .addClass('ultimate-insert-images-left');
    };
    ...
    Images.prototype.editorSerialize = function () {
        ...
        $.each(data, function (key) {
            ...
            $data.find('.ultimate-insert-images').find('figcaption, figure').removeAttr('contenteditable');
            $data.find('.ultimate-insert-images-progress').remove();
    ...
    Images.prototype.uploadAdd = function (e, data) {
        ...
        $place.addClass('ultimate-insert-images');
    ...
    Images.prototype.uploadProgressall = function (e, data) {
        ...
        if (this.options.preview === false) {
            ...
            $progressbar = this.$el.find('.ultimate-insert-active').find('progress');
    ...
    Images.prototype.uploadProgress = function (e, data) {
        ...
        if (this.options.preview) {
            ...
            $progressbar = data.context.find('.ultimate-insert-images-progress');
        ...
        if (this.options.preview && data.context) {
            ...
            if (this.options.autoGrid && $place.find('figure').length >= this.options.autoGrid) {
                $.each(this.options.styles, function (style, options) {
                    var className = 'ultimate-insert-images-' + style;
                ...
                $place.addClass('ultimate-insert-images-grid');
    ...
    Images.prototype.selectImage = function (e) {
        ...
        if (this.core.options.enabled) {
            ...
            $image.addClass('ultimate-insert-image-active');
            $image.closest('.ultimate-insert-images').addClass('ultimate-insert-active');
    ...
    Images.prototype.unselectImage = function (e) {
        var $el = $(e.target),
            $image = this.$el.find('.ultimate-insert-image-active');

        if ($el.is('img') && $el.hasClass('ultimate-insert-image-active')) {
            $image.not($el).removeClass('ultimate-insert-image-active');
            $('.ultimate-insert-images-toolbar, .ultimate-insert-images-toolbar2').remove();
            this.core.removeCaptions($el);
            return;
        }

        $image.removeClass('ultimate-insert-image-active');
        $('.ultimate-insert-images-toolbar, .ultimate-insert-images-toolbar2').remove();

        if ($el.is('.ultimate-insert-caption-placeholder')) {
    ...
    Images.prototype.removeImage = function (e) {
        var images = [],
            $selectedImage = this.$el.find('.ultimate-insert-image-active'),
        ...
        if (e.which === 8 || e.which === 46) {
            ...
            if (selection && selection.rangeCount) {
                ...
                caretPosition = ultimateEditor.selection.getCaretOffsets(current).left;
                ...
                if ($sibling && $sibling.hasClass('ultimate-insert-images')) {
                    images.push($sibling.find('img'));
                }

                // If text is selected, find images in the selection
                selectedHtml = ultimateEditor.selection.getSelectionHtml(document);
                if (selectedHtml) {
                    $('<div></div>').html(selectedHtml).find('.ultimate-insert-images img').each(function () {
                        images.push($(this));
                    });
                }
            }
            ...
            if (images.length) {
                for (i = 0; i < images.length; i++) {
                    ...
                    $parent = images[i].closest('.ultimate-insert-images');
    ...
    Images.prototype.addToolbar = function () {
        var $image = this.$el.find('.ultimate-insert-image-active'),
            $p = $image.closest('.ultimate-insert-images'),
            active = false,
        ...
        $toolbar = $('.ultimate-insert-images-toolbar');
        $toolbar2 = $('.ultimate-insert-images-toolbar2');

        $toolbar.find('button').each(function () {
            if ($p.hasClass('ultimate-insert-images-' + $(this).data('action'))) {
    ...
    Images.prototype.repositionToolbars = function () {
        var $toolbar = $('.ultimate-insert-images-toolbar'),
            $toolbar2 = $('.ultimate-insert-images-toolbar2'),
            $image = this.$el.find('.ultimate-insert-image-active'),
        ...
        if ($toolbar.length) {
            if ($image.closest('.ultimate-insert-images-grid-active').length) {
                $image = $image.closest('.ultimate-insert-images-grid-active');
            }
    ...
    Images.prototype.toolbarAction = function (e) {
        ...
        $ul = $li.closest('ul');
        $lis = $ul.find('li');
        $p = this.$el.find('.ultimate-insert-active');
        ...
        $lis.find('button').each(function () {
            var className = 'ultimate-insert-images-' + $(this).data('action');
    ...
    Images.prototype.toolbar2Action = function (e) {
        ...
        if (callback) {
            callback(this.$el.find('.ultimate-insert-image-active'));
        }
...
})(jQuery, window, document, ultimateEditor.util);
